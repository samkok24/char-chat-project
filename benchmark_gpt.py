#!/usr/bin/env python3
"""GPT 모델 벤치마크 — gpt-4.1, gpt-4.1-mini, gpt-5-mini, gpt-5.1"""
...
async def run_10(session, token, model, sub_model):
    print(f"\n{'='*60}")
    print(f"  {sub_model}")
    print(f"{'='*60}")
...
def print_summary(sub_model, results):
    ok = [r for r in results if r["chars"] > 0]
    if not ok:
        print(f"  {sub_model}: ALL FAILED (0/10)")
        return
    ...
    print(f"  {sub_model} ({len(ok)}/10 성공):")
...
async def main():
    print(f"[{datetime.now().strftime('%H:%M:%S')}] GPT 모델 벤치마크 시작")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for model, sub_model in MODELS:
            results = await run_10(session, token, model, sub_model)
            all_results[sub_model] = results

        print(f"\n\n{'='*70}")
        print("GPT 모델 벤치마크 종합")
        print(f"{'='*70}")
        for _, sub_model in MODELS:
            if sub_model in all_results:
                print_summary(sub_model, all_results[sub_model])
