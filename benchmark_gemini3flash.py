#!/usr/bin/env python3
"""Gemini 3 Flash Preview 벤치마크 (10회)"""
...
async def main():
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Gemini 3 Flash Preview benchmark (10 runs)")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.put(f"{BASE}/me/model-settings",
                               params={"model": "gemini", "sub_model": "gemini-3-flash-preview", "response_length": "medium"},
                               headers={"Authorization": f"Bearer {token}"}) as r:
            if r.status != 200:
                body = await r.text()
                print(f"Model set FAILED: {r.status} {body[:200]}")
                return
        print("Model set: gemini-3-flash-preview")
        ...
        print(f"\n{'='*60}")
        print(f"  Gemini 3 Flash Preview ({len(ok)}/10)")
        print(f"{'='*60}")
