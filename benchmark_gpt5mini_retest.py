#!/usr/bin/env python3
"""GPT-5-mini reasoning_effort=low 재테스트"""
...
async def main():
    print(f"[{datetime.now().strftime('%H:%M:%S')}] GPT-5-mini reasoning_effort=low retest")
    async with aiohttp.ClientSession() as session:
        ...
        async with session.put(f"{BASE}/me/model-settings",
                               params={"model": "gpt", "sub_model": "gpt-5-mini", "response_length": "medium"},
                               headers={"Authorization": f"Bearer {token}"}) as r:
            print(f"model setting: {r.status}")
        ...
        print(f"\n{'='*60}")
        print(f"GPT-5-mini reasoning_effort=low ({len(ok)}/10 success):")
        if ok:
            ...
            print("  ALL FAILED")
        print(f"{'='*60}")
        print(f"\nbefore: ALL FAILED 0/10 -> after: {len(ok)}/10 success")
