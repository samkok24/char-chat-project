  ...
  return (
    <section className="mt-8">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold text-white">웹툰 TOP10 <span className="ml-2 text-xs text-gray-400 align-middle">(조회수 기준)</span></h2>
      </div>
      <ErrorBoundary>
        <ul className="grid grid-cols-7 gap-4">
          {isLoading && Array.from({ length: 14 }).map((_, idx) => (
            <WebtoonSkeleton key={idx} />
          ))}
          {!isLoading && !isError && !empty && data.map((story) => (
            <WebtoonItem key={story.id} story={story} />
          ))}
