...
- **생성 모달(ImageGenerateModal)**
  - 프롬프트 프리필
    ...
    - 원작챗: 캐릭터 정보 + 원작 세계관
  - 옵션
    - 모델 선택: provider + model (예: stability/sdxl, replicate/flux, openai/dalle3 등)
    - 개수: 1~8(기본 4)
...
- **상세페이지 전용 “이미지 생성/삽입” 모달**
  ...
  - 생성 결과 “스테이징” 영역(중요)
    - 생성이 끝난 이미지들은 우선 스테이징 그리드에 쌓임(아직 갤러리에 부착 X)
    - 선택 UI: 개별 체크박스 + “전체선택/해제” 토글
    - 액션 버튼
      - 개별 삽입, 선택 삽입, 전체 삽입
      - 개별 삭제, 선택 삭제, 전체 삭제(스테이징에서만 제거; 이미 갤러리에 붙은 자산엔 영향 없음)
    - 용량 가드: 갤러리 상한(n) 초과 시, 삽입 시도에서 남은 슬롯만큼만 삽입하고 결과를 토스트로 집계

- **갤러리 노출**
  - 상세 상단: 대표이미지
  - 본문: 미니갤러리(최대 6), 클릭 시 확대 뷰어
  - 카드/그리드: 대표이미지 즉시 반영(react-query invalidate)
  - 다장 보유 시 캐러셀(화살표) 지원: 좌/우 이동, 키보드 ←/→, 모바일 스와이프, 인디케이터 dot/카운트
  - 프리로드: 다음/이전 한 장 미리 로딩으로 전환 속도 개선
...
- 생성/잡
  - POST `/media/generate` → { job_id }
    - body: { 
        ...
        provider, model, 
        ratio?, width?, height?, steps?, guidance_scale?, sampler?, seed?
      }
...
- 조회/관리
  ...
  - BULK: `DELETE /{entity}/{id}/images?ids=a,b,c` (다중 분리/삭제)

- 삽입/업로드(신규)
  - POST `/{entity}/{id}/images/attach` → 기존 `asset_id`들을 연결(내 자산/URL로 생성된 자산)
  - POST `/{entity}/{id}/images/attach-bulk` → { asset_ids:[...] } (전체/선택 삽입)
  ...
  - DELETE `/media/assets` → { asset_ids:[...] } (스테이징 자산 다중 삭제)
...
- **비용/크레딧**: 장당 차감(설정값), 실패/차단 시 자동 환불
 - **스테이징 TTL**: entity에 attach되지 않은 자산은 24h 후 정리(크론 잡)
 - **프로바이더/모델 추상화**:
   - providers: stability, replicate, openai, local(선택)
   - models 예시: sdxl-1.0, flux-1-dev, dalle-3, local-sdxl
   - 공통 파라미터 맵핑 레이어(steps, guidance_scale, sampler, size)
...
- 상세/채팅 연동
  - 상세: 미니갤러리 + 캐러셀 화살표(components/ui/carousel 사용), 썸네일 프리로드
  - 채팅방: 헤더/사이드 미니갤러리(2장 이상일 때 노출), 화살표/스와이프, 현재 세션에서 선택 이미지 유지
- 상세 헤더: "대표이미지 생성" 버튼(오너만 렌더)
- 캐시 무효화: 생성/대표지정/삭제/정렬 후 `['character', id] | ['story', id]` 등 invalidate
 - **모델 선택 UI**: 기존 `ModelSelectionModal` 재사용 또는 모달 내 인라인 셀렉트
   - fields: provider, model, resolution preset(1:1/3:4/16:9), steps, guidance
   - 최근 선택 기억(localStorage) + 프로젝트 기본값 오버라이드
...
- 프롬프트 프리셋 저장/불러오기(선택)
 - 모델별 제한/비용 표(예시)
   - dalle-3: 1024 정방형 고정, 비용 높음, 검열 강함
   - sdxl: 자유 비율/스텝, 비용 중간, 속도 빠름
   - flux: 스타일리시, 베타, 속도/품질 트레이드오프
 - 레이트 리밋: provider 별 초당 요청/동시 작업 제한 적용
...
- [ ] 콘텐츠 가드 차단 시 메시지와 환불 처리 확인
 - [ ] 다중 선택/전체 선택/전체 삽입·삭제 액션의 부분 성공/실패 집계 토스트 확인
 - [ ] 상세/채팅 캐러셀 키보드/스와이프 네비게이션, 포커스링 a11y 확인
 - [ ] 모델 전환 시 파라미터 맵핑 정상(steps/size/guidance), 최근 선택 복원 확인
