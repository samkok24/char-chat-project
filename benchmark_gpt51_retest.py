#!/usr/bin/env python3
"""GPT-5.1 reasoning_effort=low 재테스트"""
...
async def main():
    print(f"[{datetime.now().strftime('%H:%M:%S')}] GPT-5.1 reasoning_effort=low 재테스트")
    async with aiohttp.ClientSession() as session:
        ...
        async with session.put(f"{BASE}/me/model-settings",
                               params={"model": "gpt", "sub_model": "gpt-5.1", "response_length": "medium"},
        ...
        print(f"\n{'='*60}")
        print(f"GPT-5.1 reasoning_effort=low 결과 ({len(ok)}/10 성공):")
        if ok:
            ...
            print("  ALL FAILED")
        print(f"{'='*60}")
        print(f"\n비교: medium TTFB=12.72s -> low TTFB=???")
