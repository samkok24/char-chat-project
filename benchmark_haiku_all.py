#!/usr/bin/env python3
"""Claude Haiku 3종 비교: 3 Haiku vs 3.5 Haiku vs Haiku 4.5"""
...
MODELS = [
    ("claude", "claude-3-haiku-20240307", "Claude 3 Haiku"),
...
def print_summary(label, results, cost_per_turn_won):
    ...
    print(f"    Cost:    {cost_per_turn_won:.1f} won/turn")


async def main():
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Haiku 3 vs 3.5 vs 4.5 benchmark")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # API pricing: input $/1M, output $/1M
        costs = {
            "claude-3-haiku-20240307": ("$0.25/$1.25", 1.7),
            "claude-3-5-haiku-20241022": ("$0.80/$4.00", 5.6),
            "claude-haiku-4-5-20251001": ("$1.00/$5.00", 6.9),
        }

        print(f"\n\n{'='*70}")
        print("Haiku 3 vs 3.5 vs 4.5 comparison")
        print(f"{'='*70}")
        for _, sub_model, label in MODELS:
            if sub_model in all_results:
                cost_won = costs.get(sub_model, ("?", 0))[1]
                print_summary(all_results[sub_model][0], all_results[sub_model][1], cost_won)

        print(f"\n{'='*70}")
        print("Cost summary")
        print(f"{'='*70}")
        for _, sub_model, label in MODELS:
            price_str, won = costs[sub_model]
            print(f"  {label}: {price_str} per 1M -> {won:.1f} won/turn")
